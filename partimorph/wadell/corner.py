"""Corner circle fitting for roundness calculation.

Includes embedded n-sphere fitting (BSD-2, scikit-guess).
"""
...
def compute_corner_circles(
    ...
    """Fit circles to consecutive convex points of the boundary.

    Args:
        convex_points: (N, 2) convex keypoints.
        keypoints: (M, 2) all keypoints.
        r_max: Radius of the maximum inscribed circle.
        r_max_pos: (2,) center of the maximum inscribed circle.
        circle_fit_thresh: Threshold ratio for validating circle fit (0-1).

    Returns:
        (radii, centers) — arrays of fitted corner circle radii and centers.
    """
...
def _nsphere_fit(x: np.ndarray) -> tuple[float, np.ndarray]:
    """Fit a circle (2D n-sphere) using the Coope method with scaling.

    BSD-2 License, scikit-guess project by Joseph Fox-Rabinovitz.
    """
